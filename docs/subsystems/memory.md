...
`dsh-tool-memory` registers the `memoryCatalog` session projection, `stateVersion: 2`, whose state is `{ taken: boolean }` with `init: () => ({ taken: false })`. It folds `step/start` to `{ taken: true }`, this plugin's own `user/message` with `source.kind === 'tool-memory'` and `form === 'snapshot'` to `{ taken: true }`, and `compaction/summary` to `{ taken: false }`. The prepended `agent/pre-step` listener awaits `next()` first, then injects at most once per surface generation while `taken` is false: it renders `visible(cwd)` through `renderSnapshot` and `MemoryStore.scan`, and appends a sourced `user/message` after the claimed user batch. An empty store at that first step injects nothing. Every injected snapshot is a logged `user/message`, so replay rebuilds each model request from the log.
