...
这三个工具读写由 dsh-memory 拥有的持久记忆存储；会话工作目录决定项目作用域。注入的目录是受 `injectMaxBytes` 约束、来源为插件的 user/message，因此本目录记录随产品发布的预算与回忆上限。
