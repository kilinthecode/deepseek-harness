...
- **`dsh --profile headless --image` or an SDK image prompt is refused with "does not support image input"** — The selected route's model declares no image modality; the refusal happens before any file is read or the prompt is admitted. Select an image-capable model, as above.
