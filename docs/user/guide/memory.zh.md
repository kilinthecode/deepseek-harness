...
会话开始时，模型会收到一份它可见的已保存记忆目录：每条记忆占一行，列出类型、名称和一行描述。当已保存的记忆发生变化时，目录在后续某轮开始时刷新；上下文压缩之后也会再次刷新。模型用 `memory_recall` 读取一条记忆的完整内容，用 `memory_write` 保存或替换一条记忆，用 `memory_forget` 删除一条记忆。
...
项目 `<slug>` 是项目目录名加上由其完整路径派生的八个十六进制字符。用任意编辑器编辑文件，或删除文件以忘掉该记忆；运行中的进程在下次打开存储时才会看到变化，因此手工编辑后请重启长期运行的 `dsh web` 宿主。无法解析的文件会在存储打开时被移到一旁，命名为 `<name>.json.bak.<timestamp>`，其他记忆仍然可用。[存储包 README](../../../packages/memory/memory/README.zh.md) 记录了记录的各个字段。
...
- 长期运行的 `dsh web` 宿主在启动时读取存储，因此其他进程（例如旁边的一次 headless 运行）写入的记忆只有重启后才会出现。
