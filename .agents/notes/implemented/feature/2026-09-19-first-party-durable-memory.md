...
**`memory/write` and `memory/forget` session events with an invariant companion.** They would give UI rendering and replay a dedicated record of each mutation. They are unnecessary for reconstructability: the tool call and result already record each mutation, and the store is cross-session state rather than session state. Without them the package needs no invariant companion, no persistence-change record, and no persistence-catalog regeneration. A future memory panel reads the store through a host controller.
