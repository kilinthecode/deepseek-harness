...
**Re-inject the catalog after every write, gated by a generation counter.** Re-sending the whole catalog after a write the model just made costs tokens for no information. A frozen snapshot taken once per surface generation, recorded in the [frozen snapshot and review note](2026-09-25-frozen-memory-snapshot-and-review.md), covers this process's writes without a second copy in the same conversation.
