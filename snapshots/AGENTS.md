...
Every Session compares against its own committed request headers: a child inherits its parent's effective route, so its header records what that session's adapter supplied, and the Session comparison already rejects any header change. A scenario whose manifest declares child Session roles gets three times the loader-smoke budget, because provisioning a peer outlives the primary Session's first turn.
