...
每个 `session/modelCatalog` 模型行携带一个可选的 `inputModalities` 列表，复制自 LLM registry 解析出的模型信息：缺省表示能力未知，列出但不含 `'image'` 则标记该路由仅支持文本。catalog 成员关系仍是建议性的；prompt 端点自身的已解析模型检查才是针对图片类请求的唯一强制点。
