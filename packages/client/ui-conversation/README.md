...
A plugin the composer cannot import (`ui-model-selection`) advises whether the Session's current model route accepts images through `ctx.conversation.routeImage`, mirroring the composer-block push: `false` refuses new image intake with `image.modelUnsupported`; while the rail holds images under a `false` route, whether a route switch left them there or a restored draft brought them back, the composer toasts the same copy once and refuses Send and the Enter gesture for a message draft or for a claimed command that carries attachments (`/goal`, `/plan`). An unclaimed `/` line and a claim without attachments stay submittable under the command plane's own attachment policy; the command submit refuses a line that adjudicates to an attachment-carrying claim before encoding its images. The model seat and a `/model` menu pick switch the route without submitting the draft. `null` or an unknown or unlisted route allows images and leaves the decision to Host prompt admission.
...
- **Command images have no Host capability check** — Host prompt admission refuses an image the Session's resolved model does not accept, but Host command execution admits the images of a command declaring `input.attachments` (`/goal`, `/plan`) without consulting the route. The composer refusal under a `false` route-image advisory is the only guard on that path, so a `null` advisory (catalog not loaded, unlisted selection, or no `ui-model-selection`) lets such a command carry images to a text-only route.
