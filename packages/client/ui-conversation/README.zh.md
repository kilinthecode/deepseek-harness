...
composer 无法导入的插件（`ui-model-selection`）通过 `ctx.conversation.routeImage` 通知 Session 当前模型路由是否接受图片，做法与 composer block 的推送方式一致：`false` 会用 `image.modelUnsupported` 拒绝新的图片入口；在 `false` 路由下 rail 中仍有图片时（无论是路由切换后留下的，还是恢复的草稿带回的），composer 会弹出同样的提示一次、禁用发送，并拒绝消息草稿的 Enter 手势（`/` 命令仍可提交，以便用 `/model` 切换回来）；`null` 或未知、未列出的路由都允许图片，与 Host 的 prompt 准入保持一致。
