...
- **A projection records that the opportunity was taken.** The `memoryCatalog` projection is `stateVersion: 2` with `{ taken: boolean }`. It folds `step/start` and this plugin's own snapshot messages to `{ taken: true }`, and `compaction/summary` to `{ taken: false }`. After the first step of a surface generation the listener does not read the store.
...
The listener is prepended on `agent/pre-step`, awaits the rest of the chain first (so a `compaction/summary` in that chain can fold `taken` back to `false` in the same step), and appends the snapshot to an `enter` decision. It runs once per step, not per retry. If `state.taken` is true or `injectMaxBytes` is `0`, it returns the decision unchanged and does not read the store. Otherwise it renders `visible(cwd)` with `renderSnapshot` and appends a user-role message after the claimed user batch and runtime context. The `memoryCatalog` projection is `stateVersion: 2` with `{ taken: boolean }` and `init: () => ({ taken: false })`. It folds `step/start` to `{ taken: true }`, this plugin's own `user/message` with `source.kind === 'tool-memory'` and `form === 'snapshot'` to `{ taken: true }` (a fork child's seed may carry the parent's snapshot without the parent's `step/start` rows), and `compaction/summary` to `{ taken: false }`. The snapshot message carries `source: { kind: 'tool-memory', form: 'snapshot', sections: [{ name: 'memory-catalog', text }] }`. The `tool-memory` kind is attribution-only: a reader without this plugin keeps the message and its source fields.
