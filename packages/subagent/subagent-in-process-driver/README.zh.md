...
2. 在第一个 await 之前，捕获父级委派的沙箱与审批覆盖项，并一次性解析子 agent 的 Agent 选项；此后父级的切换不会到达子 agent。
3. 提示词包含图片块时，在 `ctx.agents.create()` 之前检查该解析出的子级路由：声明的输入模态省略 `image` 的路由会以 `SubagentError` 错误码 `MODEL_DOES_NOT_SUPPORT_IMAGES` 拒绝，且不会创建子 agent；提供方、模型或 `llm` 服务缺失，或路由从未披露其模态时，则放行。对于声明 `imageInput: false` 的提供方，subagent 服务在调用该提供方之前就已拒绝图片提示词。
4. 通过宿主 agent 工厂，以同一份解析出的 Agent 选项创建子 agent，并把调用方必需的信号传入创建事务。
5. 在该事务未发布的设置窗口内，安装请求的 persona、工具限制与结构化输出运行时。
6. 发布子 agent，保留返回的句柄，并驱动一项任务。
7. 从完整的自有运行中读取子 agent 自身的输出——最后一条非空 assistant 消息，若无则取其累积的 assistant 文本——以及最终持久化的轮次原因，并排除任何 fork 初始内容。
