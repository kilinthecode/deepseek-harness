...
对于每个已接受的 `session/prompt`，文本和持久内容引用会原样进入一条用户消息。内联 `SdkEncodedImageBlock` 会先通过组合中的附件存储完成校验与提交，因此会话日志保留内容寻址的图片引用而不是 base64 字节。当提示包含任意图片块（无论是内联编码还是已持久化）时，若初始化路由解析出的模型声明的输入模态列表不包含 `image`，则在产生任何会话或附件副作用之前拒绝该提示；未声明模态的路由则予以放行。此包不会添加系统提示词文本或工具 schema；这些内容来自组合中的其他插件。
