...
| [`src/content.ts`](src/content.ts) | Shared content helpers: attachment access mapping, content walks, image-input support, session image id resolution, file projection, image offload accounting, offloaded-image projection, and tool-update projection. `resolveImageAttachmentRefs()` returns the cited ids' references from user content and tool-result images (first occurrence wins) plus the ids no image carries; `resolveDelegationImages()` validates a delegation tool's `images` argument (non-empty, no duplicates, within the per-message limit, every id known) and returns fresh image blocks without `offloaded`. |
...
- **Image projection follows the captured route** — durable `ImageBlock` references become route-specific request versions only for image-capable models; text-only models receive stable placeholders that name the full attachment identity and a read-only path when access resolves.
