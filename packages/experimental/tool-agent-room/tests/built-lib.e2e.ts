/** Plain-Node smoke for the built room tool plugin bundle. */
...
const requiredArtifact = 'packages/experimental/tool-agent-room/lib/index.js'

describe.skipIf(!existsSync(artifact(requiredArtifact)))('Agent room built LIB tools', () => {
  it('imports the built bundle and keeps the exported function plugin', async () => {
    ...
    // silently loses the room commands.
    const script = `
