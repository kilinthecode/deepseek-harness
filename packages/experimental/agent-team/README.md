...
- **Image route checks precede the durable append, and the Lead's route can change after them** — `sendMessage()` and `spawnTeammate()` check the route before their Team transaction, and the transaction does not repeat the check, which awaits a model-info read. The Lead's route changes when the Lead logs a request on a different model, and such a change after the check has two failure directions. If the Lead target's route becomes text-only, the queued image is still delivered through `Agent.steer()`, which applies no image gate, and the Lead's model sees placeholder text instead of the image. If a spawn's inherited route becomes text-only, the subagent service's own creation check refuses the child after the `provisioning` record, so the Team records a `failed` member and the name and a member slot are used up. A change toward an image-capable model only produces a refusal the caller can retry.
