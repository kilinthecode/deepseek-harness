...
- **创建 teammate**——`spawn_teammate` 接收名字、描述与初始任务，以及可选的 `images` 会话图片附件 id 列表（追加在提示词之后）；只有 Lead 可以调用它。
- **发送消息**——`send_message` 在最近的步骤边界对运行中的成员进行 steering（中途引导）、启动或恢复非活动成员，并接受同样的可选 `images` 列表（追加在消息之后）。
- **查看与等待**——`list_agents` 返回各成员的 `target`、可用状态以及图像能力（`acceptsImages`）；`wait_agent` 等待下一次团队变化；`interrupt_agent` 停止 teammate 的当前轮次（仅限 Lead）。
...
一段共享 system 策略会说明显式 delegation 要求、共享 cwd 行为、文件陈旧版本恢复、Bash／formatter／codegen 风险、task／write-scope 协调、Steer 投递、mailbox 不重试规则、Lead 必须在回答前等待，以及图片只发给 `acceptsImages` 不是 `unsupported` 的成员。Lead 与 teammate 的全部九个 Team schema 相同；执行时检查仅限 Lead 的操作权限。`list_agents` 行包含可选的 `acceptsImages`（`supported`、`unsupported` 或 `undeclared`），来自每个成员的实时 LLM 路由，解析失败时省略该字段。`spawn_teammate` 在初始 user 消息前加上 `<system-reminder>\nYou are teammate "<name>".\nYour Team Lead is named "lead".\nUse list_agents({}) to find your teammates and their names.\nTo message your Team Lead, use send_message({ target: "lead", message: "..." }).\nTo message another teammate, use send_message({ target: "<teammate name>", message: "..." }).\n</system-reminder>`，接着是一个空行和任务。该前缀不含 Team id，禁用运行时上下文时也能生效。fork 继承历史，不额外添加 Lead 身份消息。可选的 `images` 参数——`spawn_teammate` 上为「Attachment ids of images already shown in this conversation, appended to the prompt.」，`send_message` 上为「Attachment ids of images already shown in this conversation, appended to the message.」——会在任何持久化成员或 mailbox 工作之前对照调用方的会话解析。
