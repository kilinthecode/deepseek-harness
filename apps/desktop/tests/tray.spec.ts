...
it('shows the application icon with its name as the tooltip and an Open / Quit menu', () => {
  ...
  expect(f.native.setToolTip).toHaveBeenCalledWith('Portal Harness')
  expect(labels(native.menus[0]!)).toEqual(['Open Portal Harness', 'separator', 'Quit Portal Harness'])
...
it('relabels the menu in the current locale and ignores relabel after disposal', () => {
  ...
  f.setLocale('zh')
  f.tray.relabel()
  expect(labels(native.menus[1]!)).toEqual(['打开 Portal Harness', 'separator', '退出 Portal Harness'])
  f.tray.dispose()
  f.tray.dispose()
